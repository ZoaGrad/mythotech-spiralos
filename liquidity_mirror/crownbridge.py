...
class CrownBridge:
    ...
    def create_key_share(self, branch: GovernanceBranch, branch_member_id: str) -> MPCKeyShare:
        """Create new MPC key share"""
        share = MPCKeyShare(branch=branch, branch_member_id=branch_member_id)
    ...
    def sign_transaction(self, tx_id: str, share_id: str) -> Optional[CryptographicSignature]:
        """
        ...
        signature_data = hashlib.sha256(f"{message_hash}:{share.share_data}".encode()).hexdigest()
    ...
    def execute_bridge_transaction(self, tx_id: str) -> bool:
        """
    ...
    def get_bridge_stats(self) -> Dict:
        ...
        return {
            ...
            "success_rate": (
                self.successful_transactions / self.total_transactions * 100 if self.total_transactions > 0 else 0
            ),
...
def example_crownbridge():
    ...
    bridge.get_branch_shares(GovernanceBranch.EXECUTIVE)
