...
try:
    ...
    from holoeconomy.empathy_market import EmpathyMarket
...
def test_scarindex_panic_threshold():
    ...
    assert (
        ScarIndexOracle.PANIC_THRESHOLD == 0.67
    ...
    print("✓ Coherence status thresholds correct")
...
def test_consensus_4_of_5_quorum():
    ...
    external_id = next(oid for oid, o in council.oracles.items() if o.provider == "external_validator")
    ...
    non_commercial_ids = [
        oid
        for oid, o in council.oracles.items()
        if o.provider_type == ProviderType.NON_COMMERCIAL and oid != external_id
    ]
    commercial_ids = [oid for oid, o in council.oracles.items() if o.provider_type == ProviderType.COMMERCIAL]
    ...
    if non_commercial_ids and len(commercial_ids) >= 2:
        ...
        if external_id not in votes_3_yes_no_external:
            assert (
                arbitration is True
            ), f"3-of-5 approvals without external validator should require arbitration, got: {reason}"
...
def test_f2_right_of_refusal():
    ...
    print("✓ HTTP 403 response correct")
    ...
    print("✓ Refusal stored in system")
...
def test_f2_appeal_72_hour_sla():
    ...
    print("✓ Appeal not overdue initially")
...
def test_emp_burn_validation():
    ...
    print("✓ GBE initialized with threshold 0.7")
    ...
    validation_valid = market.validate_burn(
        ...
        relational_context={"permits_burn": True, "reason": "Mutual agreement to transmute EMP value"},
    )
    ...
    validation_no_permit = market.validate_burn(
        token_id="token_003",
        amount=Decimal("50"),
        witness_declarations=["Witness 1 statement", "Witness 2 statement"],
...
def test_emp_burn_coherence_threshold():
    ...
    validation = market.validate_burn(
        token_id="token_004",
        amount=Decimal("100"),
        witness_declarations=["Valid witness statement one", "Valid witness statement two"],
        relational_context={"permits_burn": True},
    )
