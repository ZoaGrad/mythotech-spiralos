    ...
    def get_http_response(self) -> Dict:
        ...
        return {
            ...
            "appeal_route": {
                ...
                "instructions": self.appeal_instructions
                or (
...
class Judge:
    ...
    def review_case(self, case: JudicialCase) -> JudicialCase:
        ...
        self.average_review_time = (self.average_review_time * self.cases_reviewed + review_time) / (
    ...
    def _review_crisis_escalation(self, case: JudicialCase) -> JudicialCase:
        ...
        if scarindex < self.scarindex_critical_threshold:
            ...
            case.reasoning = (
                f"ScarIndex ({scarindex:.4f}) in warning zone. " "Conditional approval for targeted remediation."
            )
            case.conditions = [
            ...
            case.remediation_required = ["Execute phases 1-4 of Recovery Protocol", "Reduce system load by 50%"]
        else:
            # False alarm or recovering
            case.verdict = JudgmentVerdict.APPROVED
            case.reasoning = f"ScarIndex ({scarindex:.4f}) within acceptable range. " "Crisis escalation not warranted."
    ...
    def _review_resource_audit(self, case: JudicialCase) -> JudicialCase:
        ...
        if scarindex >= self.scarindex_optimal_threshold and resource_efficiency >= 0.7:
            case.verdict = JudgmentVerdict.APPROVED
            case.reasoning = (
                f"Resource allocation optimal. ScarIndex: {scarindex:.4f}, " f"Efficiency: {resource_efficiency:.4f}"
            )
        elif scarindex >= self.scarindex_critical_threshold:
            case.verdict = JudgmentVerdict.CONDITIONAL
            case.reasoning = f"Resource allocation acceptable but suboptimal. " f"ScarIndex: {scarindex:.4f}"
            case.conditions = ["Optimize resource allocation within 24 hours", "Target efficiency >= 0.7"]
        else:
            case.verdict = JudgmentVerdict.REJECTED
            case.reasoning = f"Resource allocation critically inefficient. " f"ScarIndex: {scarindex:.4f}"
            case.remediation_required = [
    ...
    def _review_lineage_evaluation(self, case: JudicialCase) -> JudicialCase:
        ...
        if cmp >= self.cmp_minimum and residue < self.residue_threshold:
            ...
            case.verdict = JudgmentVerdict.CONDITIONAL
            case.reasoning = f"Lineage marginally productive. CMP: {cmp:.4f}"
            case.conditions = ["Improve CMP by 20% within 3 generations", "Reduce residue accumulation"]
        else:
            case.verdict = JudgmentVerdict.REJECTED
            case.reasoning = f"Lineage unproductive. CMP: {cmp:.4f} below minimum " f"({self.cmp_minimum})"
            case.remediation_required = [
                "Terminate lineage",
                "Cleanup accumulated residue",
    ...
    def _review_constitutional_compliance(self, case: JudicialCase) -> JudicialCase:
        ...
        if scarindex_after > scarindex_before:
            ...
            case.verdict = JudgmentVerdict.CONDITIONAL
            case.reasoning = "Coherence stagnant. No violation but no progress."
            case.conditions = ["Next iteration must increase coherence", "Review optimization parameters"]
        else:
            case.verdict = JudgmentVerdict.REJECTED
            case.reasoning = (
                "Constitutional violation. Law of Recursive Alignment broken. "
    ...
    def _review_residue_cleanup(self, case: JudicialCase) -> JudicialCase:
        ...
        if total_residue <= cleanup_target:
            case.verdict = JudgmentVerdict.APPROVED
            case.reasoning = (
                f"Residue within acceptable limits. " f"Total: {total_residue:.4f}, Target: {cleanup_target:.4f}"
            )
        elif total_residue <= cleanup_target * 1.5:
            case.verdict = JudgmentVerdict.CONDITIONAL
            case.reasoning = f"Residue elevated but manageable. " f"Total: {total_residue:.4f}"
            case.conditions = ["Execute cleanup within 24 hours", "Reduce residue to target level"]
        else:
            case.verdict = JudgmentVerdict.ESCALATED
            case.reasoning = f"Residue critically high. " f"Total: {total_residue:.4f}, Target: {cleanup_target:.4f}"
            case.remediation_required = [
    ...
    def _review_holon_termination(self, case: JudicialCase) -> JudicialCase:
        ...
        if cmp < self.cmp_minimum and (residue > self.residue_threshold or efficiency < 0.3):
            case.verdict = JudgmentVerdict.APPROVED
            case.reasoning = (
                f"Holon termination justified. CMP: {cmp:.4f}, " f"Residue: {residue:.4f}, Efficiency: {efficiency:.4f}"
            )
            case.remediation_required = [
            ...
            case.verdict = JudgmentVerdict.CONDITIONAL
            case.reasoning = "Holon performance marginal. Grant probation period."
            case.conditions = [
            ...
            case.verdict = JudgmentVerdict.REJECTED
            case.reasoning = f"Holon termination not justified. CMP: {cmp:.4f} acceptable."
...
class JudicialSystem:
    ...
    def _initialize_default_judges(self):
        ...
        # Crisis Judge
        crisis_judge = Judge(name="Crisis Judge Alpha", specialization=JudgmentType.CRISIS_ESCALATION)
        self.judges[crisis_judge.id] = crisis_judge

        # Resource Judge
        resource_judge = Judge(name="Resource Judge Beta", specialization=JudgmentType.RESOURCE_AUDIT)
        self.judges[resource_judge.id] = resource_judge

        # Constitutional Judge
        constitutional_judge = Judge(
            name="Constitutional Judge Gamma", specialization=JudgmentType.CONSTITUTIONAL_COMPLIANCE
        )
    ...
    def review_all_pending(self) -> List[JudicialCase]:
        ...
        # Sort by priority
        pending_sorted = sorted(self.pending_cases, key=lambda cid: self.cases[cid].priority.value)
    ...
    def invoke_right_of_refusal(
        ...
        refusal = RightOfRefusal(
            ...
            appeal_instructions=(
                "You may appeal this refusal by submitting to /api/v1.5/dissent "
                "with grounds for your appeal. Review guaranteed within 72 hours."
            ),
        )
    ...
    def file_appeal(
        self, refusal_id: str, appellant_id: str, grounds: str, evidence: Optional[Dict] = None
        ...
        appeal = RefusalAppeal(
            refusal_id=refusal_id, appellant_id=appellant_id, grounds=grounds, evidence=evidence or {}
        )
    ...
    def review_appeal(self, appeal_id: str, reviewer_judge_id: str) -> Optional[RefusalAppeal]:
        """
    ...
    def get_overdue_appeals(self) -> List[RefusalAppeal]:
        ...
        return [appeal for appeal in self.appeals.values() if appeal.is_overdue()]
