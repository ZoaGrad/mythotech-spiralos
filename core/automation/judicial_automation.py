...
class JudicialAutomation:
    ...
    async def fetch_pending_cases(self) -> List[Case]:
        ...
        try:
            response = (
                ...
                .select("id, case_number, case_type, complexity, filing_date, assigned_judge_id, status")
                .eq("status", "pending")
                .execute()
            )
    ...
    async def assign_cases_to_judges(self, judges: List[Judge], cases: List[Case]) -> Dict[str, str]:
        ...
        sorted_judges = sorted(judges, key=lambda j: (-j.f2_score, j.current_workload))
    ...
    async def persist_assignments(self, assignments: Dict[str, str]) -> bool:
        ...
        try:
            for case_id, judge_id in assignments.items():
                ...
                judge_response = self.supabase.table("judges").select("current_workload").eq("id", judge_id).execute()

                if judge_response.data:
                    current_workload = judge_response.data[0]["current_workload"]
                    self.supabase.table("judges").update(
                        {"current_workload": current_workload + 1, "updated_at": datetime.now(timezone.utc).isoformat()}
    ...
    async def run(self):
        ...
        try:
            ...
            if assignments:
                ...
                if success:
                    logger.info(
                        f"Successfully completed judicial automation. " f"Assigned {len(assignments)} cases to judges."
                    )
                else:
