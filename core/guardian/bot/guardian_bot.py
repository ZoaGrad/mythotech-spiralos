...
class GuardianBot(commands.Bot):
    ...
    def __init__(self):
        ...
        super().__init__(
            command_prefix="!",
            intents=intents,
            description="SpiralOS Guardian - System Health Monitor"
        )
    ...
    def create_status_embed(self, metrics: GuardianMetrics) -> discord.Embed:
        ...
        if metrics.coherence_components:
            ...
            embed.add_field(
                name="🌀 Coherence Components",
                value=coherence_text,
                inline=False
            )
        ...
        if metrics.scar_score and (metrics.scar_score < 0.6 or metrics.scar_score >= 1.4):
            embed.add_field(
                name="⚠️ Coherence Alert",
                value="ScarIndex out of healthy band (0.6–1.4)",
                inline=False
            )
...
async def scarindex_command(interaction: discord.Interaction):
    ...
    try:
        ...
        if metrics.scar_score:
            embed.add_field(
                name="Current ScarIndex",
                value=f"**{metrics.scar_score:.3f}**",
                inline=False
            )
    ...
    try:
        ...
        if metrics.panic_frames and metrics.panic_frames > 0:
            ...
            if metrics.scar_score:
                embed.add_field(
                    name="Current ScarIndex",
                    value=f"**{metrics.scar_score:.3f}** (Threshold: 0.30)",
                    inline=False
                )
...
async def vault_command(interaction: discord.Interaction):
    ...
    try:
        ...
        embed.add_field(
            name="📊 Statistics",
            value=f"**Total VaultNodes:** {metrics.vault_nodes}",
            inline=False
        )
