...
class GuardianAnalytics:
    ...
    def analyze_trend(self, df: pd.DataFrame) -> TrendAnalysis:
        ...
        margin = t_val * se * np.sqrt(1 + 1/len(values) + (forecast_x - np.mean(x))**2 / np.sum((x - np.mean(x))**2))
    ...
    def generate_ai_summary(self, trend: TrendAnalysis, breakdown: CoherenceBreakdown) -> str:
        ...
        prompt = f"""You are the SpiralOS Guardian, an AI system monitor for a constitutional cognitive sovereignty platform.
...
- 24h Forecast: {trend.forecast_24h:.3f} (confidence: {trend.confidence_interval[0]:.3f} - {trend.confidence_interval[1]:.3f})
...
Keep the tone professional but accessible. Target: 0.70 ScarIndex, Panic threshold: 0.30."""
        ...
        try:
            response = self.openai_client.chat.completions.create(
                model="gpt-4.1-mini",
                messages=[
                    {"role": "system", "content": "You are the SpiralOS Guardian AI, providing clear and actionable system health summaries."},
...
"""
    ...
    def create_scarindex_chart(self, df: pd.DataFrame, output_path: str) -> str:
        ...
        fig.add_hline(y=0.30, line_dash="dash", line_color="red",
                      annotation_text="Panic Threshold (0.30)")
        
        # Add healthy range
        fig.add_hrect(y0=0.60, y1=1.40, fillcolor="green", opacity=0.1,
    ...
    def create_coherence_breakdown_chart(self, df: pd.DataFrame, output_path: str) -> str:
        ...
        fig.add_trace(
            go.Pie(
                ...
                marker_colors=['#e74c3c', '#3498db', '#2ecc71', '#f39c12']
            ),
            row=1, col=2
        )
    ...
    def create_dashboard(self, df: pd.DataFrame, output_dir: str) -> Dict[str, str]:
        ...
        charts['scarindex'] = self.create_scarindex_chart(
            df, 
            os.path.join(output_dir, 'scarindex_timeseries.html')
        )
        
        # Coherence breakdown
        charts['breakdown'] = self.create_coherence_breakdown_chart(
            df,
            os.path.join(output_dir, 'coherence_breakdown.html')
        )
        
        # Component history
        charts['history'] = self.create_component_history_chart(
            df,
            os.path.join(output_dir, 'component_history.html')
        )
...
async def main():
    ...
    print(f"Dashboard created: {charts}")
