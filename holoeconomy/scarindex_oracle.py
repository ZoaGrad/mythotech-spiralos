...
# --- FastAPI Application ---
app = FastAPI(
    title="SpiralOS ScarIndex Oracle", description="Provides access to the ScarIndex coherence metric.", version="1.0.0"
)
...
async def get_latest_scarindex():
    ...
    """
    response = (
        supabase.table("scarindex_calculations")
...
def main():
    ...
    parser.add_argument(
        "--compute", action="store_true", help="Run a single computation and exit without starting the server."
    )
    parser.add_argument(
        "--port", type=int, default=int(os.environ.get("PORT", 8000)), help="Port to run the server on."
    )
