...
class EAFInterpreter:
    ...
    def policy_to_control(self, policy: PolicyStatement) -> Tuple[Dict[str, ControlParameters], str]:
        """
        ...
        if policy.statement in self.policy_to_control_rules:
            ...
            raise ValueError(f"No mapping rule for policy: {policy.statement}. " f"Manual interpretation required.")

    def control_to_policy(self, control_params: Dict[str, float]) -> Tuple[str, str]:
        """
    ...
    def validate_alignment(
        self, control_action: Dict[str, float], policy_statement: str, audit_logs: Dict
        ...
        try:
            policy = PolicyStatement(statement=policy_statement, principle="", priority=1, source="Oracle Council")
            ...
            if param_name in control_action:
                ...
                if diff > 0.20:  # 20% threshold
                    discrepancies.append(
                        f"{param_name}: expected {expected.value}, got {actual} " f"(diff: {diff:.2f})"
                    )
            else:
    ...
    def check_recursive_alignment(
        self, operation_type: str, stated_value: str, actual_outcome: Dict
    ) -> Tuple[bool, str]:
        """
    ...
    def check_reflexive_validation(self, alignment_claim: str, audit_evidence: Dict) -> Tuple[bool, float]:
        """
    ...
    def detect_failure_modes(
        self, coherence: CoherenceScores, recursive_aligned: bool, reflexive_validated: bool
    ...
    def calculate_legitimacy_score(
        ...
        L_base = w1 * coherence.c_operational + w2 * coherence.c_audit + w3 * coherence.c_constitutional
        ...
        trace = {
            ...
            "alignment_checks": {
                "recursive_alignment": {
                    ...
                    "description": (
                        "Operations serve stated values" if recursive_aligned else "Operations diverge from values"
                    ),
                },
                "reflexive_validation": {
                    "verified": reflexive_validated,
                    "description": (
                        "Audit confirms alignment" if reflexive_validated else "Audit does not confirm alignment"
                    ),
                },
            },
...
if __name__ == "__main__":
    ...
    current_params = {"volatility_threshold": 0.04, "exploration_rate": 0.08}
    ...
    coherence = CoherenceScores(c_operational=0.85, c_audit=0.88, c_constitutional=0.72)

    recursive_aligned = True
    reflexive_validated = True

    failure_modes, severity = interpreter.detect_failure_modes(coherence, recursive_aligned, reflexive_validated)
    ...
    trace = interpreter.generate_justification_trace(
        coherence, recursive_aligned, reflexive_validated, failure_modes, legitimacy_score, classification
    )
