...
from .f2_judges import JudgePriority, JudgmentType, JudicialSystem
...
class SpiralOSv1_1:
    ...
    def __init__(
        ...
        if enable_soc:
            self.pid_controller = SOCPIDController(
                target_scarindex=target_scarindex, target_tau=target_tau, kp=1.0, ki=0.5, kd=0.2
            )
        else:
            from ache_pid_controller import AchePIDController

            self.pid_controller = AchePIDController(target_scarindex=target_scarindex)
    ...
    async def transmute_ache_holonic(
        ...
        holon = self.holon_stack.create_holon(
            holon_type=holon_type, task_description=f"Transmute {source}: {content.get('description', 'N/A')}"
        )

        # Step 2: Execute Holon
        holon_result = await self.holon_stack.execute_holon(
            holon_id=holon.id,
            input_data={"ache_before": ache_before, "scarindex": 0.7, "content": content},  # Initial estimate
            optimal_scarindex=optimal_scarindex,
        )

        # Step 3: Calculate ScarIndex
        # Simplified - in production would use actual semantic analysis
        components = CoherenceComponents(narrative=0.75, social=0.70, economic=0.65, technical=0.80)

        ache_after = ache_before * 0.5  # Simplified transmutation

        ache_measurement = AcheMeasurement(before=ache_before, after=ache_after)

        scarindex_result = self.oracle.calculate(components=components, ache=ache_measurement)

        # Step 4: Update SOC PID Controller
        if self.enable_soc:
            event_size = abs(ache_before - ache_after)
            guidance_scale, soc_state = self.pid_controller.update_soc(scarindex_result.scarindex, event_size)
        else:
        ...
        if self.enable_panic_frames and self.panic_manager:
            if self.panic_manager.should_trigger(scarindex_result.scarindex):
                panic_frame = self.panic_manager.trigger_panic_frame(
                    ...
                    metadata={"scarindex_id": scarindex_result.id, "holon_id": holon.id, "source": source},
                )
...
async def example_spiralos_v1_1():
    ...
    spiralos = SpiralOSv1_1(
        target_scarindex=0.7, target_tau=1.5, enable_judges=True, enable_panic_frames=True, enable_soc=True
    )
