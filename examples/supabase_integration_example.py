...
class SpiralOSClient:
    ...
    def __init__(self):
        ...
        if missing:
            raise ValueError(
                ...
                "Set them with: export SUPABASE_URL=... SUPABASE_KEY=..."
            )

        self.client: Client = create_client(self.url, self.key)
        print(f"✓ Connected to SpiralOS at {self.url}")

    def create_ache_event(self, source: str, content: Dict, ache_level: float) -> str:
        ...
        """
        response = (
            self.client.table("ache_events")
            .insert({"source": source, "content": content, "ache_level": ache_level})
            .execute()
        )
    ...
    def calculate_scarindex(self, event_id: str) -> Dict:
        ...
        """
        response = self.client.rpc("coherence_calculation", {"event_id": event_id}).execute()

        calc = response.data
        print(f"✓ ScarIndex calculated: {calc['scarindex']:.4f}")
        print(
            f"  Components: N={calc['c_narrative']:.2f}, S={calc['c_social']:.2f}, "
            f"E={calc['c_economic']:.2f}, T={calc['c_technical']:.2f}"
        )
        print(
            f"  Ache transmutation: {calc['ache_before']:.2f} → {calc['ache_after']:.2f} "
    ...
    def mint_scarcoin(self, calc_id: str) -> Optional[Dict]:
        ...
        response = (
            self.client.table("smart_contract_txns")
            .select("*")
            .eq("to_state", calc_id)
            .eq("txn_type", "MINT")
            .execute()
    ...
    def seal_vaultnode(self, ref_id: str, ref_type: str, commit_sha: Optional[str] = None) -> Dict:
    ...
    def get_oracle_status(self) -> Dict:
        ...
        """
        response = self.client.table("scar_index_oracle_sync").select("*").execute()
    ...
    def get_system_health(self) -> Dict:
        ...
        """
        response = self.client.table("system_health").select("*").execute()
    ...
    def get_active_panic_frames(self):
        ...
        response = self.client.table("active_panic_frames").select("*").execute()
...
def demo_complete_flow():
    ...
    client.seal_vaultnode(ref_id=event_id, ref_type="demo_event", commit_sha="demo_abc123")
    ...
    client.calculate_scarindex(event_id2)
